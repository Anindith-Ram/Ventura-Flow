...
def run(paper: dict, research_brief: str, logger=None) -> str:
    ...
    output = call_llm(MODEL, BEAR_ANALYST_SYSTEM_PROMPT, user_msg, temperature=0.6)
    log(f"Done. Output length: {len(output)} chars")
    return output


if __name__ == "__main__":
    ap = argparse.ArgumentParser(description="Run Bear Analyst standalone")
    ap.add_argument("--paper", default="sample_input.json", help="Path to paper JSON")
    ap.add_argument("--brief", default=None, help="Path to bear brief markdown (defaults to latest in output-dir)")
    ...
    if args.brief:
        ...
        candidates = sorted(output_dir.glob("*_bear_brief.md"))
        if not candidates:
            print("ERROR: no bear brief found. Run bear_researcher.py first, or pass --brief.")
            sys.exit(1)
    ...
    critique = run(paper, brief, logger=log)
    out_path = output_dir / f"{run_id}_bear_critique.md"
    out_path.write_text(critique)
    log(f"Critique saved to {out_path.name}")
